...
class GCNResnet(nn.Module):
    def __init__(self, representation, num_classes, freezed_layer, in_channel=300, t=0, adj_file=None, pretrained=False):
        ...
        # define the parameter of the GGNN class
        self.opt = {}
        self.opt["state_dim"] = 310
        self.opt["annotation_dim"] = 10
        self.opt["n_edge_types"] = 1
        self.opt["n_node"] = 10
        self.opt["n_steps"] = 10
        
        
        self.ggnn = GGNN(self.opt)
        self.classi = nn.Linear(2048*110, num_classes)
        self.sigmoid = nn.Sigmoid()
        adj = create_adjacency_matrix(num_classes, t, adj_file, n_nodes=10, n_edge_types=1)
        self.A = Parameter(torch.from_numpy(adj).float()).detach()  #
        
    def forward(self, x, inp, annotation):
        import pdb
        pdb.set_trace()
        feature = self.resnet_bcnn(x) # [32, 300]
        batch = feature.size(0)
        # feature = feature.view(feature.size(0), -1)
          # what is inp??  label embedding because data_loader would return "batch_size" label embedding
        
        ggcn_out = self.ggnn(inp, annotation, self.A.cuda())
