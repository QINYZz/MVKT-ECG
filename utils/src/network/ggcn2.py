...
class GGNN(nn.Module):
    ...
    def forward(self, input):
        ...
        batch_aog_nodes = input
        batch_in_matrix = self._in_matrix.repeat(batch_size, 1).view(batch_size, node_num, -1).cuda()
        batch_out_matrix = self._out_matrix.repeat(batch_size, 1).view(batch_size, node_num, -1).cuda()
        for t in range(self.time_step):
            ...
            flatten_aog_nodes = batch_aog_nodes .contiguous() .view(batch_size * node_num, -1)
