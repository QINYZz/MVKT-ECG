...
def get_basemodel(modeltype, pretrained=False):
    import pdb
    #pdb.set_trace()
    modeltype = globals()[modeltype]  # return all Variables in dictionary  form
    print(modeltype)
    if pretrained == False:
...
class Basemodel(nn.Module):
    ...
    def __init__(self, modeltype, pretrained=False):
        ...
        if modeltype.startswith('multi'):
            basemodel = self._reconstruct_multi_label_model(basemodel)
        ...
        if modeltype.startswith('ECG'):
            basemodel = self._reconstruct_mbn(basemodel)  #
        self.features = basemodel.features
        self.BiRNN = nn.LSTM(512 * 4, 150, batch_first=True, bidirectional=True)
    ...
    def _reconstruct_multi_label_model(self, basemodel):
        model = nn.Module()
        model.features = nn.Sequential(*list(basemodel.children())[:-2])
        model.representation = basemodel.maxpool2
        model.classifier = basemodel.fc
        model.representation_dim = basemodel.fc.weight.size(1)
        return model
    ...
    def _reconstruct_mbn(self, basemodel):
        model = nn.Module()
        model.features = basemodel
        model.representation = nn.AdaptiveAvgPool1d(1)
        model.classifier = nn.Linear(basemodel.feature_dim, 9)
        model.representation_dim = model.classifier.weight.size(1)  # ???,
        return model
